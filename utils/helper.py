...
class P4InfoHelper(object):
    ...
    def buildMeterConfEntry(self):

        meter_config = p4runtime_pb2.MeterConfig()

        meter_config.cir = 1
        meter_config.cburst = 1
        meter_config.pir = 1
        meter_config.pburst = 1

        return meter_config

    def buildMeterEntry(self, meter_name, meter_config):
        meter_entry = p4runtime_pb2.MeterEntry(config=meter_config,
                                               meter_id=self.get_id(name=meter_name, entity_type="meters"),
                                               index=p4runtime_pb2.Index(index=1))
        # print(dir(p4runtime_pb2.Index))
        print(dir(meter_entry))
